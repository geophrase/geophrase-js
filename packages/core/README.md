...
The snippet below uses `mode: 'server'` so you can paste it into an HTML file and see the widget end-to-end **without creating an API key first**.
...
- **`client`**: the widget resolves the address in the browser and passes it straight to `onSuccess`. Requires `key` in the options. You should restrict that key to your domain(s) in the [Geophrase Business Dashboard](https://business.geophrase.com).
